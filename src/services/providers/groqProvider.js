export function createGroqProvider() {
  return {
    ...
    async testConnection(providerSettings) {
      const apiKey = providerSettings.groqApiKey;
      const model = providerSettings.groqModel || 'llama-3.3-70b-versatile';

      if (!apiKey) {
        return { ok: false, error: 'Groq: API key is required.' };
      }
      ...
      try {
        ...
        if (!res.ok) {
          let msg = `Groq responded with status ${res.status}.`;
          try {
            const errData = JSON.parse(res.text);
            if (errData.error?.message) {
              msg = `Groq: ${errData.error.message}`;
            }
          } catch {}
        ...
        const msg = normalizeFetchError(err, 'Groq');
        return { ok: false, error: msg };
      }
    },
